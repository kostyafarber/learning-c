...
get_curr_dir_attrs(void)
{
    ...
    child = malloc(sizeof(struct stat));
    if (child == NULL)
        sys_error("malloc");
    ...
    struct dir_attr_check *dir_attrs = malloc(sizeof(struct dir_attr_check));
    if (dir_attrs == NULL)
        return NULL;
...
void traverse_directories(ino_t child, ino_t parent)
{
    ...
    traverse_directories(curr_dir_attrs->i_node_child, 
                            curr_dir_attrs->i_node_parent);
}

int main(int argc, char **argv)
{
    /* TODO add code to return to directory after traversal */
    struct dir_attr_check *curr_dir_attrs = get_curr_dir_attrs();
    if (curr_dir_attrs == NULL) {
        printf("Failed to allocate pointer");
        exit(EXIT_FAILURE);
